...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		} else $data['error_email'] = '';	

		if (isset($this->error['enquiry'])) {
			$data['error_enquiry'] = $this->error['enquiry'];
		} else $data['error_enquiry'] = '';	
		...
		$data['map'] = html_entity_decode($this->config->get('config_map'));
		$data['telephone'] = $this->config->get('config_telephone');
		$contacts = explode(';',$this->config->get('config_contacts'));
		$data['contacts'] = array();
		foreach($contacts as $e) {
			$e = explode('=',$e);
			if(isset($e[2])) $data['contacts'][] = array('type' => $e[0],'name' =>$e[1],'value' => $e[2]);
		}
		
		if(count($data['contacts']) > 0){
			usort($data['contacts'], create_function('$a,$b','return strcmp($a["name"], $b["name"]);'));
		}
		$data['details'] = nl2br($this->config->get('config_details')); 
		$data['open'] = $this->config->get('config_open');
		$data['contact_text'] = $this->config->get('config_contact_text');


		if (isset($this->request->post['name'])) {
			$data['name'] = $this->request->post['name'];
		} else $data['name'] = $this->customer->getFirstName();

		if (isset($this->request->post['email'])) {
			$data['email'] = $this->request->post['email'];
		} else $data['email'] = $this->customer->getEmail();
