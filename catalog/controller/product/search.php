		...
		if(isset($this->request->get['keyword']) ) {
			...
			if(strlen($keywords) >= 2) {
				...
				foreach( $parts as $part ) {
					...
					$add .= ' OR LOWER(model) LIKE "%' . $this->db->escape($part) . '%")';
				}
				$add = substr($add, 4);
				$sql  = 'SELECT product_id, name, model FROM ' . DB_PREFIX . 'product WHERE ' . $add . ' AND status=1 ORDER BY sort_order ASC, LOWER(name) ASC, LOWER(model) ASC LIMIT 10';
				$res = $this->db->query($sql);
				if($res) {
					$data = (isset($res->rows)) ? $res->rows : $res->row;
					// For the seo url stuff
					$basehref = 'product/product&keyword=' . urlencode($this->request->get['keyword']) . '&product_id=';
