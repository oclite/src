...
class ModelCheckoutOrder extends Model {	
		...
		foreach ($data['custom_field'] as $custom_field) {
			...
			if ($custom_field_query->num_rows) {
				...
				if ($custom_field_query->row['type'] == 'select' || $custom_field_query->row['type'] == 'radio') {
					$custom_field_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "custom_field_value cfv LEFT JOIN " . DB_PREFIX . "custom_field_value_description cfvd ON (cfv.custom_field_value_id = cfvd.custom_field_value_id) WHERE cfv.custom_field_id = '" . (int)$custom_field['custom_field_id'] . "' AND cfv.custom_field_value_id = '" . (int)$custom_field['value'] . "'");
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			$data['title'] = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_id);
			
			$data['text_greeting'] = sprintf($language->get('text_new_greeting'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
			...
			$data['logo'] = $this->config->get('config_url') . 'content/' . $this->config->get('config_logo');
			$data['customer_id'] = $order_info['customer_id'];
			$data['link'] = HTTP_SERVER . 'index.php?route=account/order/info&order_id=' . $order_id;
			...
			$data['comment'] = ($comment && $notify)?	nl2br($comment) : '';
			...
			if ($this->config->get('config_order_mail')) {
				...
				$mail->setFrom($this->config->get('config_shop_email'));
		...
		if ($order_info && $order_info['order_status_id']) {
			...
			if ($notify) {
				...
				if ($order_info['customer_id']) {
					...
					$message .= HTTP_SERVER . 'index.php?route=account/order/info&order_id=' . $order_id . "\n\n";
				}
