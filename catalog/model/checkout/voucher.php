...
class ModelCheckoutVoucher extends Model {
	...
	public function getVoucher($code) {
		...
		$voucher_query = $this->db->query("SELECT *, vt.name AS theme FROM " . DB_PREFIX . "voucher v LEFT JOIN " . DB_PREFIX . "voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) WHERE v.code = '" . $this->db->escape($code) . "' AND v.status = '1'");
	...
	public function confirm($order_id) {
		...
		if ($order_info) {
			...
			$voucher_query = $this->db->query("SELECT *, vt.name AS theme FROM `" . DB_PREFIX . "voucher` v LEFT JOIN " . DB_PREFIX . "voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) WHERE v.order_id = '" . (int)$order_id . "'");
			...
			foreach ($voucher_query->rows as $voucher) {
				...
				if (is_file(DIR_CONTENT . $voucher['image'])) {
					$template->data['image'] = $this->config->get('config_url') . 'content/' . $voucher['image'];
				} else {
					$template->data['image'] = '';
				}
