...
class ControllerCatalogFilter extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['filter_group_name'])) {
			$data['filter_group_name'] = $this->request->post['filter_group_name'];
		} elseif (isset($this->request->get['filter_group_id'])) {
			$data['filter_group_name'] = $this->model_catalog_filter->getFilterGroupDescriptions($this->request->get['filter_group_id']);
		} else {
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['filter_group_name']) < 1) || (utf8_strlen($this->request->post['filter_group_name']) > 64)) {
			$this->error['group'] = $this->language->get('error_group');
		}
		if (isset($this->request->post['filter'])) {
			foreach ($this->request->post['filter'] as $filter_id => $filter) {
				if ((utf8_strlen($filter['name']) < 1) || (utf8_strlen($filter['name']) > 64)) {
