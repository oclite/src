...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if (isset($result['images'][0]) && is_file(DIR_CONTENT . $result['images'][0])) {
				$image = $this->model_tool_image->resize($result['images'][0], 100, 100);
			} else $image = '';
		...
		$data['sort_name'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.name' . $url, 'SSL');
	...
	protected function getForm() {
		...
		$data['entry_qpbox'] = $this->language->get('entry_qpbox');
		...
		$data['error_date_available'] = (isset($this->error['date_available']))? $this->error['date_available'] : '';
		...
		if (isset($this->request->post['seourl'])) {
			...
			$data['seourl'] = $product_info['seourl'];
		} else $data['seourl'] = '';
		...
		} else $data['date_available'] = date('Y-m-d');
		...
		if (isset($this->request->post['qpbox'])) {
			$data['qpbox'] = $this->request->post['qpbox'];
		} elseif (!empty($product_info)) {
			$data['qpbox'] = $product_info['qpbox'];
		} else $data['qpbox'] = 1;
		...
		} else $data['subtract'] = 1;

		if (isset($this->request->post['sort_order'])) {
			$data['sort_order'] = $this->request->post['sort_order'];
		} elseif (!empty($product_info)) {
			$data['sort_order'] = $product_info['sort_order'];
		} else $data['sort_order'] = 1;
		...
		if (isset($this->request->post['weight_class_id'])) {
			...
			$data['weight_class_id'] = $product_info['weight_class'];
		} else {
		...
		if (isset($this->request->post['length_class_id'])) {
			...
			$data['length_class_id'] = $product_info['length_class'];
		} else {
		...
		} else $data['date_start'] = '0000-00-00';
		
		if (isset($this->request->post['date_end'])) {
			$data['date_end'] = $this->request->post['date_end'];
		} elseif (!empty($product_info)) {
			$data['date_end'] = $product_info['date_end'];
		} else $data['date_end'] = '0000-00-00';
		...
		} else $product_images = array();

		$data['product_images'] = array();

		foreach ($product_images as $product_image) {
			if (!is_file(DIR_CONTENT . $product_image)) {
				$product_image = '';
			}

			$data['product_images'][] = array(
				'image'      => $product_image,
				'thumb'      => $this->model_tool_image->resize($product_image, 100, 100)
			);
		}
		...
		} else $data['product_layout'] = array();
