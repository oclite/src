...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_CONTENT . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else $directory = DIR_CONTENT . 'catalog';
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'name'  => implode(' ', $name),
					'type'  => 'directory',
					'path'  => utf8_substr($image, utf8_strlen(DIR_CONTENT)),
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . utf8_substr($image, utf8_strlen(DIR_CONTENT . 'catalog/')) . $url, 'SSL')
				);
				...
				//
				$data['images'][] = array(
					'thumb' => $this->model_tool_image->resize(utf8_substr($image, utf8_strlen(DIR_CONTENT)), 100, 100),
					'name'  => implode(' ', $name),
					'type'  => 'image',
					'path'  => utf8_substr($image, utf8_strlen(DIR_CONTENT)),
					'href'  => $server . 'content/' . utf8_substr($image, utf8_strlen(DIR_CONTENT))
				);
			}
		}
	...
	public function delete() {
		...
		foreach ($paths as $path) {
			$path = rtrim(DIR_CONTENT . str_replace(array('../', '..\\', '..'), '', $path), '/');

			// Check path exsists
			if ($path == DIR_CONTENT . 'catalog') {
