...
class ControllerSaleOrder extends Controller {
  	...
  	public function delete() {
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $order_id){
				$this->model_sale_order->deleteOrder(intval($order_id));
			}
	...
	public function getForm() {
		...
		} else $data['order_id'] = 0;
		if ($this->request->server['HTTPS']) {
			$data['store_url'] = HTTPS_CATALOG;
		} else $data['store_url'] = HTTP_CATALOG;
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($store_info) {
					...
					$store_email = $store_info['config_shop_email'];
					$store_telephone = $store_info['config_telephones'];
					$store_fax = $store_info['config_fax'];
				} else {
					$store_address = $this->config->get('config_address');
					$store_email = $this->config->get('config_shop_email');
