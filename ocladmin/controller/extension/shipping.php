...
class ControllerExtensionShipping extends Controller {
	...
	public function toggle() {
		...
		if ($this->validate()) {
			$this->load->model('setting/setting');
			$s = $this->model_setting_setting->getSetting($this->request->get['extension']);
			$s[$this->request->get['extension'].'_status'] = (1 === (int)$s[$this->request->get['extension'].'_status'])? 0 : 1;
			$this->model_setting_setting->editSetting($this->request->get['extension'], $s);
			$this->response->redirect($this->url->link('extension/shipping', 'token=' . $this->session->data['token'], 'SSL'));
		}
		$this->getList();
	}
		...
		if ($files) {
			foreach ($files as $file) {
				...
				$data['extensions'][] = array(
					...
					'status'     => $this->config->get($extension . '_status') ? 'on' : 'off',
					'status_text'   => $this->config->get($extension . '_status') ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
					'status_toggle' => $this->url->link('extension/shipping/toggle', 'token=' . $this->session->data['token'] . '&extension=' . $extension, 'SSL'),
