...
class ModelSaleVoucher extends Model {
	...
	public function getVouchers($data = array()) {
		$sql = "SELECT v.voucher_id, v.code, v.from_name, v.from_email, v.to_name, v.to_email, (SELECT vt.name FROM " . DB_PREFIX . "voucher_theme vt WHERE vt.voucher_theme_id = v.voucher_theme_id) AS theme, v.amount, v.status, v.date_added FROM " . DB_PREFIX . "voucher v";
	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
				...
				if ($voucher_theme_info && is_file(DIR_CONTENT . $voucher_theme_info['image'])) {
					$data['image'] = HTTP_CATALOG . 'content/' . $voucher_theme_info['image'];
				} else {
					$data['image'] = '';
				}
