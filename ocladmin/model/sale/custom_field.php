...
class ModelSaleCustomField extends Model {
	...
	public function getCustomField($custom_field_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "custom_field` cf WHERE cf.custom_field_id = '" . (int)$custom_field_id . "'");
	...
	public function getCustomFields($data = array()) {
		...
		if (isset($data['filter_name']) && $data['filter_name'] !== null) {
			$sql .= " WHERE cf.name LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
