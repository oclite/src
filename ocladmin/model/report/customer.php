...
class ModelReportCustomer extends Model {
	public function getOrders($data = array()) { 
		$sql = "SELECT c.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cg.name AS customer_group, c.status, COUNT(o.order_id) AS orders, SUM(op.quantity) AS products, SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id)LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group` cg ON (c.customer_group_id = cg.customer_group_id) WHERE o.customer_id > 0";
		
		if (!empty($data['filter_order_status_id'])) {
			$sql .= " AND o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else $sql .= " AND o.order_status_id > '0'";
	...
	public function getRewardPoints($data = array()) { 
		...
		$wh = ' WHERE';
		if (!empty($data['filter_date_start'])) {
			$sql .= $wh . " AND DATE(cr.date_added) >= '" . $this->db->escape($data['filter_date_start']) . "'";
			$wh = '';
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= $wh . " AND DATE(cr.date_added) <= '" . $this->db->escape($data['filter_date_end']) . "'";
		}
	...
	public function getTotalRewardPoints($data = array()) {
		...
		if ($implode) $sql .= " WHERE " . implode(" AND ", $implode);
	...
	public function getCredit($data = array()) { 
		$sql = "SELECT ct.customer_id, CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, .name AS customer_group, c.status, SUM(ct.amount) AS total FROM `" . DB_PREFIX . "customer_transaction` ct LEFT JOIN `" . DB_PREFIX . "customer` c ON (ct.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group` cg ON (c.customer_group_id = cg.customer_group_id)";
		$wh = ' WHERE';
		if (!empty($data['filter_date_start'])) {
			$sql .= $wh . "DATE(ct.date_added) >= '" . $this->db->escape($data['filter_date_start']) . "'";
			$wh = '';
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= $wh . "DATE(ct.date_added) <= '" . $this->db->escape($data['filter_date_end']) . "'";
		}
